...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group relative bg-white/90 backdrop-blur-sm rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border border-blue-100/50">
      ...
      {/* یہاں priority بالکل نہیں ہونی چاہیے جب fill استعمال کر رہے ہو */}
      ...
      <div className="p-7 space-y-4">
        ...
        <p className="text-sm font-light text-slate-600 line-clamp-2">
          {product.description}
        </p>
        ...
        <div className="pt-3 border-t border-slate-100">
          ...
          {product.perBottlePrice && (
            <p className="text-xs uppercase text-green-600 font-semibold mt-1">
        ...
        {item ? (
          <div className="flex items-center justify-between bg-gradient-to-r from-blue-50 to-cyan-50 rounded-2xl p-3.5">
            <button
              onClick={() => addToCart(item, -1)}
              className="w-11 h-11 rounded-full bg-white shadow-md text-red-500 hover:scale-110 transition"
            >
              <Minus className="w-5 h-5" />
            </button>
            <span className="text-2xl font-light">{item.quantity}</span>
            <button
              onClick={() => addToCart(item, 1)}
              className="w-11 h-11 rounded-full bg-white shadow-md text-green-500 hover:scale-110 transition"
            >
          ...
          <button
            onClick={handleAdd}
            disabled={isAdding}
            className={`w-full py-4 rounded-2xl font-medium uppercase flex items-center justify-center gap-2 transition-all ${
              isAdding
